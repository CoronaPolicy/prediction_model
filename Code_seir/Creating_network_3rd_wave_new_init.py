...
if __name__ == "__main__":
    ...
    N = 10000
    start_day = 7
    INIT_ISYM = int(0.5 * (13045 * N / 9e6))
    INIT_R = int(316528 * N / 9e6)
    INIT_D = int(2592 * N / 9e6)
    INIT_EXP = int((13045 / 9e6) * N) - INIT_ISYM
    ...
    for number_of_runs in range(intial, intial + decision):
        ...
        delta_prec = np.array([-pre_school - 0.5 * two_grades - work, -0.5 * two_grades, -0.5 * two_grades,
                               -0.5 * two_grades, 0, + 0.1 * restaurants + 0.5 * two_grades * 2,
                               +0.5 * two_grades, +0.5 * work, +0.5 * two_grades,
                               -pre_school - 0.5 * work, -airport, -0.5 * two_grades * 2,
                               -holiday, -0.9 * restaurants - 0.5 * two_grades, -airport, 0])
        ...
        percentage_out = (percentage_out - np.min(percentage_out)) / np.ptp(percentage_out)
        percentage_out = normal * (1 - percentage_out) + max_value * (percentage_out)

        percentage_out = numpy.append(percentage_out, quarantine_per)
        add_message(str(percentage_out))
        ...
        model = ExtSEIRSNetworkModel(G=G_baseline, p=P_GLOBALINTXN,
                                     ...
                                     initI_asym=INIT_ASYM,
                                     initH=INIT_ISYM - int(0.8 * INIT_ISYM), seed=seed, node_groups=node_groups,
                                     per_remove_vacc_edges=percentage_edges_removed_vacc, initR=INIT_R,
                                     store_Xseries=True)
