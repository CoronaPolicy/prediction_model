...
class graph_info(object):
    ...
    def houshold(self,alone_parma,couples_without_kids_param,kids_left_house_p
        ...
        household_data = {
            'alone': (0.05 * 0.9, [],[0, 0, 1 * alone_parma / 4, 2 * alone_parma / 4, 1 * alone_parma / 4, 0 * (1 - alone_parma) / 8,4 * (1 - alone_parma) / 8, 2 * (1 - alone_parma) / 8, 2 * (1 - alone_parma) / 8]),
            ...
            'couples_with_one_young_kid': (0.935 * 0.18 * 0.75, [0.7, 0.3], [0, 0, one_kid_young_per, 1 - one_kid_young_per, 0, 0, 0, 0, 0]),
            'couples_with_one_old_kid': (0.935 * 0.18 * 0.25, [0, 0.8, 0.2], [0, 0, 0, old_kids, 1 - old_kids, 0, 0, 0, 0]),
            'couples_with_two_kid': (0.935 * 0.19, [0.51, 0.46, 0.03],[0, 0, 0.05, two_kids_young, 1 - two_kids_young - two_kids_old-0.05, two_kids_old, 0, 0,0]),
            'couples_with_three_kid': (0.935 * 0.2, [0.51, 0.44, 0.05],[0, 0, 0.05, three_kids_young, 1 - three_kids_old - three_kids_young-0.05,three_kids_old, 0, 0, 0]),
            'couples_with_four_kid_pluse': (0.935 * 0.15, [0.51, 0.44, 0.05],[0, 0, 0.05, three_kids_young, 1 - three_kids_old - three_kids_young-0.05,three_kids_old, 0, 0, 0]),
                        }
    ...
    def layers_info(self,dict_ages):
        ...
        layer_info = {'0-9': {'ageBrackets': ['0-9'], 'meanDegree': dict_ages['0-9']*8.6,
                      ...
                      '40-59': {'ageBrackets': ['40-49', '50-59'], 'meanDegree': dict_ages['40-59']*13.8, 'meanDegree_CI': (dict_ages['40-59']*11.0, dict_ages['40-59']*16.6)},
