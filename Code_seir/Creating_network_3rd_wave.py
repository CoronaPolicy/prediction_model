...
def evenets_param(random=False):
    airport, two_grades, pre_school = 5, 6, 10  #5, 7, 11
    restaurants, work, starting_prec = 7, 12, 78
    holiday, quarantine_per, normal, max_value = 1, 90, 17, 81
    if random:
...
if __name__ == "__main__":
    ...
    running_info = open("General_information/parameters_vacc_policy.yaml")
    running_info = yaml.load(running_info, Loader=yaml.FullLoader)

    np.random.seed(123)
    N = 10000
    start_day = 7
    INIT_ISYM = int((13045 / 9e6) * N)  # int((9867 / 9e6) * N)  # int((13045 / 9e6) * N)
    INIT_R = int(316528 * N / 9e6)
    INIT_D = int(2592 * N / 9e6)
    exp_factor = 1  # 0.1  # np.random.random()
    INIT_EXP = 0.9 * int((9867 / 9e6) * N)   # exp_factor * INIT_ISYM
    INIT_ASYM = 0 * (1.25 * INIT_ISYM) * 0.25
    ...
    for number_of_runs in range(intial, intial + decision[0]):
        ...
        delta_prec = np.array([-pre_school - two_grades - work, -two_grades, -two_grades,
                               -two_grades, 0, +restaurants + two_grades * 3,
                               +two_grades + work, 0, +airport,
        ...
        if not General_information['Load_graph']['status']:
            ...
            demographic_graphs, individual_ageGroups, households = generate_demographic_contact_network(
                ...
                number_of_people_each_household=graph_info.numb_each_household)
        ...
        if General_information['optimize']['health_parameters']:
            ...
            p_glob_adults = 0.13
            # interaction when a person is in quarantine
            q_glob_kids = [0.1, 0.17]
            q_glob_adults = 0.06
            percentage_edges_removed_vacc = 0.8
        Q_GLOBALINTXN = []
        ...
        model = ExtSEIRSNetworkModel(G=G_baseline, p=P_GLOBALINTXN,
                                     ...
                                     initI_asym=INIT_ASYM, initR=INIT_R, initF=INIT_D,
                                     initH=INIT_ISYM - int(0.8 * INIT_ISYM), seed=1, node_groups=node_groups,
                                     per_remove_vacc_edges=percentage_edges_removed_vacc)
        ...
        if General_information['plot']['status']:
            ...
            for time, event in zip(times_for_sim, event):
                plt.axvline(int(time + start_day), 0, 1, c='k')
                plt.text(int(time+ start_day) + 0.1, 0, str(event), rotation=90)
            ...
            plt.plot(time+start_day, active_casese, '-r')
