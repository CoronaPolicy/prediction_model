        ...
        for v_type in vaccination_policies:
            for n_day in num_per_day:
                if General_information['seed']['status']:
                    ...
                    np.random.seed(seed + number_of_runs)
                    random.seed(seed + number_of_runs)
                else:
                ...
                add_message(f"vaccination policy:{v_type}")
                if v_type == "regular":
                    vaccination_data_out = vaccination_data.drop(columns=["num_tot"])
                elif v_type == "no_policy":
                    ...
                    vaccination_data_out = health_info.create_vaccination_from_scartch(N, mean_vacc_per_day=n_day,
                                                                                       std_vacc_per_day=15,
                                                                                       individual_ages_list=individual_ageGroups,
                                                                                       vacc_policy=v_type,
                                                                                       start_day=start_day)
                ...
                model = ExtSEIRSNetworkModel(G=G_baseline, p=P_GLOBALINTXN, beta_local=health_info.BETA_LOCAL,
                                             ...
                                             G_Q=graphs_for_sim[-1], q=Q_GLOBALINTXN, beta_Q=health_info.BETA_Q,
                                             isolation_time=10,
                                             initE=int(0.8 * INIT_EXP),
                                             initQ_E=INIT_EXP - int(0.8 * INIT_EXP), initI_sym=int(0.8 * 0),
                                             initI_asym=0,
                                             initH=0 - int(0.8 * 0), seed=seed+number_of_runs, node_groups=node_groups,
                ...
                if General_information['Save_model']['status']:
                    curr_date = time.strftime("%Y_%m_%d")
                    directory = f"{General_information['Save_model']['directory']}/{curr_date}/{seed+number_of_runs}/"
                    if not os.path.exists(directory):
                        os.makedirs(directory)
                    name = f"model_{v_type}_{n_day}"
                    path_name = f"{directory}/{name}"
                    with open(f"{path_name}.pickle", 'wb') as handle:
                        pickle.dump(model, handle, protocol=pickle.HIGHEST_PROTOCOL)

                    input_name = f"input_config_{v_type}_{n_day}"
                    params_name = f"input_param_file_{v_type}_{n_day}"
