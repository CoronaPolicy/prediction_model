...
def realize(n, m, k, b=0.0, alpha=0.4, beta=0.5, gamma=0.1, phi=1, r=1, q=0.5, epsilon=0.0000001, weighted=False,
...
default_ranges = {'beta': (0.5, 1, 0.05), 'k': (2, 50, 5), 'm': (2, 11, 1), 'phi': (1, 100, 10), 'r': (1, 10, 1),
                  'q': (0.0, 1, 0.1)}
default_FARZ_setting = {'seed': 1, "n": 1000, "k": 4, "m": 5, "alpha": 0.5, "gamma": 0.5, "beta": .8, "phi": 1, "o": 1,
                        'q': 0.5, "b": 0.0, "epsilon": 0.0000001, 'directed': False, 'weighted': False}
default_batch_setting = {'vari': None, 'arange': None, 'repeat': 1, 'path': '.', 'net_name': 'network', 'format': 'gml',
                         'farz_params': None}
supported_formats = ['gml', 'list']


def generate(vari=None, arange=None, repeat=1, path='.', net_name='network', format='gml',
             farz_params=default_FARZ_setting.copy()):

    random.seed(farz_params['seed'])
...
def main(argv):
    ...
    try:
        opts, args = getopt.getopt(argv, "ho:s:v:c:f:n:k:m:a:b:g:p:r:q:t:e:dw", \
                                   ["output=", "path=", "repeat=", "vary=", 'range=', 'format=', "alpha=", "beta=",
                                    "gamma=", 'phi=', 'overlap=', 'oProb=', 'epsilon=', 'cneigh=', 'directed',
                                    'weighted'])
    ...
    for opt, arg in opts:
        if opt == '-h':
            ...
            if arg in supported_formats:
                ...
                print('Format not supported , choose from ', supported_formats,
            ...
            if (arg in list(default_ranges.keys())):
                ...
                print('Invalid variable, choose form :', list(default_ranges.keys()),
